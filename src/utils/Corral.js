 ...
 */
export class Corral {
    constructor(scene, position = { x: 0, y: 0, z: 0 }, size = { width: 20, height: 2, depth: 20 }) {
        this.scene = scene;
    ...
    createFenceWalls(woodMaterial, postMaterial) {
        ...
        if (centerWallWidth > 0) {
            this.createFenceSection(
                ...
                centerWallWidth, height, plankThickness, plankHeight, spacing,
                woodMaterial,
                'front-center'
            );
        }
        ...
        this.createFenceSection(
            new THREE.Vector3(this.position.x, this.position.y + height / 2, this.position.z + depth / 2),
            width, height, plankThickness, plankHeight, spacing,
            woodMaterial,
            'back'
        );
        ...
        this.createFenceSection(
            new THREE.Vector3(this.position.x - width / 2, this.position.y + height / 2, this.position.z),
            plankThickness, height, depth, plankHeight, spacing,
            woodMaterial,
            'left'
        );
    ...
    createCornerGate(woodMaterial, gateWidth, side) {
        ...
        this.createSingleGate(woodMaterial, gateWidth, gateHeight, gateThickness, plankHeight, spacing, side);
    }
...
{{ ... }}
     */
    createSingleGate(woodMaterial, gateWidth, gateHeight, gateThickness, plankHeight, spacing, side) {
        ...
        for (let i = 0; i < numPlanks; i++) {
            const plankGeometry = new THREE.BoxGeometry(gateWidth - 0.1, plankHeight, gateThickness);
            const plank = new THREE.Mesh(plankGeometry, woodMaterial);
            plank.position.set(0, i * (plankHeight + spacing) - gateHeight / 2 + plankHeight / 2, 0);
        ...
        const frameGeometry = new THREE.BoxGeometry(gateWidth, 0.08, gateThickness * 1.5);
        ...
        const centerFrameGeometry = new THREE.BoxGeometry(0.06, gateHeight - 0.16, gateThickness * 1.5);
        ...
        for (let i = 0; i < 3; i++) {
            ...
            hinge.position.set(side === 'left' ? -gateWidth / 2 + 0.05 : gateWidth / 2 - 0.05, hingeY, 0);
    ...
    updateSingleGateCollisionBox(gateData) {
        ...
        this.collisionBoxes = this.collisionBoxes.filter(box => box.side !== `gate-${gateData.side}`);
     ...
     */
    createFenceSection(position, width, height, thickness, plankHeight, spacing, material, side) {
        const fenceGroup = new THREE.Group();
        const numPlanks = Math.floor(height / (plankHeight + spacing));
        
        // Crear tablas verticales
        for (let i = 0; i < numPlanks; i++) {
            const plankGeometry = new THREE.BoxGeometry(width, plankHeight, thickness);
            const plank = new THREE.Mesh(plankGeometry, material);
            plank.position.set(0, i * (plankHeight + spacing) - height / 2 + plankHeight / 2, 0);
            plank.castShadow = true;
            plank.receiveShadow = true;
    ...
    createAllPosts(height, radius, material) {
        ...
        for (let x = -width / 2 + postSpacing; x < width / 2; x += postSpacing) {
            if (Math.abs(x) > 0.1) { // Evitar duplicar el poste central
                this.createPost(this.position.x + x, this.position.z + depth / 2, height, radius, material, 'back-post');
            }
        }
        
        // Pared izquierda
        for (let z = -depth / 2 + postSpacing; z < depth / 2; z += postSpacing) {
            if (Math.abs(z) > 0.1) { // Evitar duplicar el poste central
                this.createPost(this.position.x - width / 2, this.position.z + z, height, radius, material, 'left-post');
            }
        }
        ...
        for (let z = -depth / 2 + postSpacing; z < depth / 2; z += postSpacing) {
            if (Math.abs(z) > 0.1) { // Evitar duplicar el poste central
                this.createPost(this.position.x + width / 2, this.position.z + z, height, radius, material, 'right-post');
            }
        }
        
        // Pared frontal (con espacio para la puerta)
        // Postes a la izquierda de la puerta
        for (let x = -width / 2 + postSpacing; x < -(gateWidth / 2); x += postSpacing) {
            this.createPost(this.position.x + x, this.position.z - depth / 2, height, radius, material, 'front-left-post');
        }
        
        // Postes a la derecha de la puerta
        for (let x = gateWidth / 2 + postSpacing; x < width / 2; x += postSpacing) {
            this.createPost(this.position.x + x, this.position.z - depth / 2, height, radius, material, 'front-right-post');
        }
        
        // Postes especiales para la puerta
        this.createPost(this.position.x - gateWidth / 2, this.position.z - depth / 2, height, radius * 1.2, material, 'gate-post');
        this.createPost(this.position.x + gateWidth / 2, this.position.z - depth / 2, height, radius * 1.2, material, 'gate-post');
    }
    ...
    applyGateRotation(gateData) {
        ...
        rotatedPosition.applyAxisAngle(new THREE.Vector3(0, 1, 0), gateData.currentRotation);
    ...
    handleFarmerInteraction(farmerPosition) {
        ...
        this.gates.forEach(gateData => {
            ...
            if (this.isFarmerNearGate(farmerPosition, gateData)) {
                console.log(`Farmer cerca de puerta ${gateData.side}. Distancia: ${distance.toFixed(2)}`);
                if (!gateData.open) {
    ...
    getClosestCollisionPoint(position, direction) {
        ...
        for (let collisionData of this.collisionBoxes) {
            const collisionPoint = this.getCollisionPoint(collisionData.box, position, direction);
            if (collisionPoint) {
