...
export class Market {
  constructor(
    ...
    // Control de visibilidad del techo
    this.roofMeshes = []; // Almacena las mallas del techo
    this.isPlayerInside = false; // Estado de si el jugador está dentro del mercado
  }
  ...
  createRoof(material) {
    ...
    // Guardar referencia del techo para control de visibilidad
  ...
  /**
   * Verifica si el jugador está dentro del área del mercado
   * @param {THREE.Vector3} position - Posición del jugador
   * @returns {boolean} - True si el jugador está dentro del mercado
   */
  isPlayerInsideMarket(position) {
    if (!position) return false;

    // Calcular los límites del mercado (ajustar según la posición real del mercado)
    const marketBounds = {
    ...
    // Verificar si la posición del jugador está dentro de los límites
    return (
  ...
  /**
   * Actualiza el estado del mercado en cada frame
   * @param {THREE.Vector3} playerPosition - Posición actual del jugador
   */
  update(playerPosition) {
    if (!playerPosition) return;

    // Actualizar animación de la puerta
    const now = Date.now();
    const dt = (now - this._lastUpdateTime) / 1000; // Convertir a segundos
    this._lastUpdateTime = now;
    this.updateDoorAnimation(playerPosition, dt);

    // Actualizar visibilidad del techo
    this.updateRoofVisibility(playerPosition);

    // Verificar proximidad del jugador para mostrar UI
